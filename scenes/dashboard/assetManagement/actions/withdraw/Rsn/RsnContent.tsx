...
import { format } from 'date-fns';
  ...
  const RsnContent = () => {
    ...
    const handleSubmit = (event: { preventDefault: () => void }) => {
      ...
      if (isFormValid && parseFloat(amount) <= maxRsnAmount) {
        setShowPopup(true);
    ...
    const handleChangeAmount = (e: { target: { value: any; }; }) => {
      ...
      if (parseFloat(value) > maxRsnAmount) {
        setAmountError(`You don't have enough RSN.`);
      } else {
        setAmountError('');
      }
      setAmount(value);
    ...
    const handleMaxClick = (e: { preventDefault: () => void; }) => {
      e.preventDefault();
      setAmount(maxRsnAmount.toString());
      setAmountError('');
    };
  ...
  return (
    <div>
      ...
      <div>
        <h2 className="text-center font-bold">113.234 RSN</h2>
      ...
      {showSuccessPopup && (
        ...
        >
          ...
          <p className="my-5 text-center text-lg font-bold">
            {amount} RSN withdrawed successfully.
          </p>
